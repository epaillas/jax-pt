...
def test_native_theory_accepts_linear_input_template(benchmark_k: np.ndarray) -> None:
    z = 0.5
    cosmo = Class()
...
def test_classpt_theory_accepts_linear_input_template(benchmark_k: np.ndarray) -> None:
    ...
    linear_input = build_linear_input_from_classy(cosmo, z=z, k=benchmark_k)
    params = make_bias_params()
    theory = ClassPTGalaxyPowerSpectrumMultipolesTheory(
...
def test_classpt_backend_requires_live_classy_cosmology(benchmark_k: np.ndarray) -> None:
    ...
    with pytest.raises(ValueError, match="requires a template built from a live classy.Class cosmology"):
        ClassPTGalaxyPowerSpectrumMultipolesTheory(template=template, k=benchmark_k)
...
def test_native_and_classpt_theory_share_high_level_api(benchmark_k: np.ndarray) -> None:
    ...
    classpt_prediction = ClassPTGalaxyPowerSpectrumMultipolesTheory(template=classpt_template, k=benchmark_k)(params)
